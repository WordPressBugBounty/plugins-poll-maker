	...
	public static function ays_poll_update_db_check() {
		...
		if(is_multisite()){
			...
			if($wpdb->get_var("SHOW TABLES LIKE '$polls_table'") != $polls_table) {
				delete_network_option($network_id, 'ays_poll_db_version');
			}

			if($wpdb->get_var("SHOW TABLES LIKE '$cats_table'") != $cats_table) {
				delete_network_option($network_id, 'ays_poll_db_version');
			}
			...
			if ( get_network_option( $network_id, 'ays_poll_db_version' ) != $ays_poll_db_version ) {
				self::activate();
				self::insert_default_values();
				update_network_option($network_id, 'ays_poll_db_version', $ays_poll_db_version);
			}
		} else {
